...
The Console owns one canonical lazy route tree for `/quality`, `/quality/monitors`, `/quality/monitors/new` and `/quality/monitors/:monitorId`. Top-level URL-backed tabs cover overview, monitors, findings, recommendation proposals, coverage and runtime. Monitor 360 covers overview, observations, evaluations, baseline history, findings, incident relationships, suppressions, definition history and evidence.
...
The feature uses `quality:read` and `monitors:read`. It polls only when explicitly configured (default off), supports manual refresh and cancels requests on route or tenant/environment changes. Capability-driven draft authoring, confirmed lifecycle actions, queued execution, baseline reset, suppression creation and recommendation decisions are supported. Autonomous remediation remains excluded.
