...
5. Exercise Kind restart/upgrade/rollback and isolated snapshot restore. Record skips as pending and resolve them before promotion.
...
7. After protected-environment approval, dispatch with `dry_run=false`. Publication is allowed only after mandatory gates pass.
