...
## Membership and dependency mutation closure

Membership, proposal, exclusion, and dependency requests bind tenant, environment, action, body, actor, reason, ETag, and a SHA-256 idempotency-key digest. Raw keys are never stored. Strict mappings reject unknown fields. OCC makes decision races deterministic; immutable pending and terminal evidence permits bounded reconciliation. Traversal enforces depth/node limits, scoped queries, cursor binding, URL-safe evidence, and explicit cycle/truncation results. Certification covers cross-tenant access, divergent replay, proposal poisoning, oversized evidence, stored XSS, cursor tampering/expiry/cross-route replay, backend-error redaction, and secret/raw-row/Kafka sentinel leakage.
