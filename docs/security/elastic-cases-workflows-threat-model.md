...
## Candidate and deadline closure

Action target selection is an authenticated, expiring server handle bound to tenant, environment, incident, action,
target, and resolution generation. The server re-resolves and reloads before preview, preventing arbitrary, stale,
or cross-scope target injection. Provider responses observed after the per-item execution deadline are treated as
unknown and cannot become verified success; lease ownership and fencing are checked on every durable transition.
