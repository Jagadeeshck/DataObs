...
_MUTATION = re.compile(
    r"\b(INSERT|UPDATE|DELETE|REPLACE|MERGE|CREATE|ALTER|DROP|TRUNCATE|RENAME|COPY|CALL|EXEC(?:UTE)?|DO|LOAD|BULK\s+INSERT|OPENROWSET|OPENDATASOURCE|OPENQUERY|LOCK\s+TABLES?|ALTER\s+SESSION|ALTER\s+SYSTEM|UNLOCK\s+TABLES?|ALTER\s+SESSION|ALTER\s+SYSTEM|GRANT|DENY|REVOKE|AUDIT|NOAUDIT|FLASHBACK|PURGE|COMMENT|SAVEPOINT|BEGIN|DECLARE|DBMS[A-Z0-9_]*|BACKUP|RESTORE|DBCC|INSTALL|UNINSTALL|VACUUM|ANALYZE|OPTIMIZE|REPAIR|REINDEX|FLUSH|RESET|KILL|SHUTDOWN|RECONFIGURE|USE|SET|BEGIN\s+TRANSACTION|START\s+TRANSACTION|COMMIT|ROLLBACK|SECURITY\s+LABEL|SESSION\s+AUTHORIZATION)\b",
    re.I,
)
