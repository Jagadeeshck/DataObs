...
INDEXES = "SELECT TABLE_SCHEMA AS `schema`,TABLE_NAME AS `table`,INDEX_NAME AS index_name,NON_UNIQUE AS non_unique,INDEX_TYPE AS index_type,SEQ_IN_INDEX AS sequence,COLUMN_NAME AS `column`,CARDINALITY AS cardinality,IS_VISIBLE AS visible,(EXPRESSION IS NOT NULL) AS functional_index FROM information_schema.statistics ORDER BY TABLE_SCHEMA,TABLE_NAME,INDEX_NAME,SEQ_IN_INDEX"
