...
class PostgresConnector:
    def __init__(
        ...
        application_name="dataobs_scanner",
    ):
        self.dsn = dsn
        self.connection = connection
        self.allow = set(allow_schemas or [])
        self.deny = set(deny_schemas or [])
        self.timeout = statement_timeout_ms
        self.application_name = application_name
        self._checkpoint = {}

    def __repr__(self):
        return f"PostgresConnector(dsn={redact(self.dsn or '')!r}, statement_timeout_ms={self.timeout})"

    def capabilities(self):
        return ConnectorCapabilities(freshness=True, profiling=True, quality=True)

    def test_connection(self):
        try:
            if self.connection and hasattr(self.connection, "execute"):
                self.connection.execute("SELECT 1")
            return ConnectionTestResult(True)
        except Exception as e:
            return ConnectionTestResult(False, str(redact(str(e))), ErrorCategory.network)

    def _filter_schema(self, schema):
        return (not self.allow or schema in self.allow) and schema not in self.deny

    def _metadata(self):
        if self.connection and hasattr(self.connection, "metadata"):
    ...
    def discover(self, request):
        ...
        for row in self._metadata():
            if self._filter_schema(row["schema"]):
                assets.append({k: row[k] for k in ("database", "schema", "table", "table_type") if k in row})
        request.metadata.ended_at = datetime.now(timezone.utc)
        return DiscoveryResult(request.metadata, assets)
        ...
        for r in self._metadata():
            if self._filter_schema(r["schema"]):
                cols.append(
                    {
                        "name": r["column"],
                        "type": normalise_type(r["type"]),
                        "native_type": r["type"],
                        "nullable": bool(r.get("nullable", True)),
                        "default": r.get("default"),
                    }
                )
        canonical = canonicalize_schema({"columns": cols})
    ...
    def profile(self, request):
        ...
        metrics = {}
        if self.connection and hasattr(self.connection, "profile"):
    ...
    def freshness(self, request):
        return FreshnessResult(request.metadata, {})

    def quality(self, request):
        return QualityResult(request.metadata, [])

    def lineage(self, request):
        return LineageResult(request.metadata, [])

    def query_history(self, request):
        return QueryHistoryResult(request.metadata, [])

    def checkpoint(self):
        return ConnectorCheckpoint("postgres", "unknown", "unknown", self._checkpoint)
    ...
    def _norm_type(t):
        ...
        if any(x in t for x in ["timestamp", "date", "time"]):
            return "timestamp"
        if any(x in t for x in ["numeric", "decimal", "double", "real"]):
            return "number"
        return t
