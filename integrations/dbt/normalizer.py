    ...
    return [
        {
            ...
            "constraints": [
                str(item.get("type") if isinstance(item, dict) else item)[:128]
                for item in (column.get("constraints") or [])[:100]
            ],
        }
    ...
    return [
        {
            ...
            for key in ("name", "type", "role", "agg", "description")
...
def _resource(
    unique_id: str, node: dict[str, Any], envelope: DbtArtifactEnvelope, limits: ArtifactLimits
) -> dict[str, Any]:
    ...
    safe = {
        "resource_id": resource_id(
            envelope.tenant_id, envelope.environment, envelope.project_id, resource_type, unique_id
        ),
        "asset_id": asset_identity(
            envelope.tenant_id, envelope.environment, envelope.project_id, resource_type, unique_id
        ),
        "unique_id": unique_id[:1024],
        "resource_type": resource_type,
        "name": bounded_text(node.get("name"), 512),
        "package": bounded_text(node.get("package_name"), 512),
        "database": bounded_text(node.get("database"), 512),
        "schema": bounded_text(node.get("schema"), 512),
        "alias": bounded_text(node.get("alias"), 512),
        "description": bounded_text(node.get("description"), limits.max_description),
        "group": bounded_text(node.get("group"), 512),
        "tags": sorted(str(tag)[:256] for tag in tags),
        ...
        "contract": {
            "enabled": bool((config.get("contract") or {}).get("enforced")),
            "alias_types": bool((config.get("contract") or {}).get("alias_types", True)),
        },
        "entities": _safe_semantic_items(node.get("entities"), limits),
        "dimensions": _safe_semantic_items(node.get("dimensions"), limits),
    ...
    if resource_type == "saved_query":
        ...
        safe["filter_fingerprint"] = (
            hashlib.sha256(str(filter_value).encode()).hexdigest() if filter_value is not None else None
        )
    if resource_type == "unit_test":
        given = node.get("given") or []
        safe["fixture_metadata"] = {
            "input_count": len(given),
            "fixture_fingerprint": hashlib.sha256(json.dumps(given, sort_keys=True).encode()).hexdigest(),
        }
    safe["definition_fingerprint"] = hashlib.sha256(
...
def parse_manifest(
    document: dict[str, Any], envelope: DbtArtifactEnvelope, limits: ArtifactLimits
) -> NormalizedArtifact:
    ...
    final = replace(
        ...
        invocation_id=str(metadata.get("invocation_id") or envelope.invocation_id)[:512],
        generated_at=metadata.get("generated_at"),
        artifact_fingerprint=hashlib.sha256(encoded).hexdigest(),
    )
    return NormalizedArtifact(final, tuple(resources))


def parse_artifact(
    document: dict[str, Any], envelope: DbtArtifactEnvelope, limits: ArtifactLimits | None = None
) -> NormalizedArtifact:
    ...
    if envelope.artifact_type == "run_results":
        ...
        for result in results:
            ...
            normalized.append(
                {
                    "unique_id": bounded_text(result.get("unique_id"), 1024),
                    "run_id": run_identity(final.tenant_id, final.environment, final.project_id, final.invocation_id),
                    "status": statuses.get(str(result.get("status")).lower(), "unknown"),
                    "execution_time": (
                        result.get("execution_time") if isinstance(result.get("execution_time"), (int, float)) else None
                    ),
                    "failure_count": result.get("failures") if isinstance(result.get("failures"), int) else None,
                    "rows_affected": (
                        response.get("rows_affected")
                        if isinstance(response, dict) and isinstance(response.get("rows_affected"), int)
                        else None
                    ),
                    "adapter_code": bounded_text(response.get("code"), 128) if isinstance(response, dict) else None,
                }
            )
    ...
    if envelope.artifact_type == "catalog":
        ...
        for collection in ("nodes", "sources"):
            for unique_id, node in (document.get(collection) or {}).items():
                ...
                output.append(
                    {
                        "unique_id": str(unique_id)[:1024],
                        "relation_type": bounded_text((node.get("metadata") or {}).get("type"), 128),
                        "columns": [
                            {
                                "name": str(name)[:512],
                                "index": col.get("index"),
                                "type": bounded_text(col.get("type"), 256),
                            }
                            for name, col in list(columns.items())[: limits.max_columns]
                        ],
                        "stats": {
                            key: value.get("value")
                            for key, value in (node.get("stats") or {}).items()
                            if key in SAFE_STATS
    ...
    output = tuple(
        {
            "source_id": bounded_text(item.get("unique_id"), 1024),
            "status": statuses.get(str(item.get("status")).lower(), "unknown"),
            "max_loaded_at": item.get("max_loaded_at"),
            "snapshotted_at": item.get("snapshotted_at"),
            "max_loaded_at_age_seconds": item.get("max_loaded_at_time_ago_in_s"),
