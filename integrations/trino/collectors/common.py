...
from integrations.trino.normalisation import resource
...
def execute(connection, registry, name, context, cfg, cluster, parameters=None):
    ...
    for row in result.rows:
        evidence = dict(zip(result.columns, row))
        yield resource(context, cluster, registry.get(name).family, "|".join(str(x) for x in row[:3]), evidence)
    if result.truncated:
