...
STATEMENTS = (
    ...
    FixedStatement("catalogs", "SELECT catalog_name FROM system.metadata.catalogs ORDER BY catalog_name", "catalogs"),
    FixedStatement(
        "cluster",
        "SELECT count(node_id) AS visible_nodes, count_if(coordinator) AS coordinators, count_if(NOT coordinator) AS workers, node_version, state FROM system.runtime.nodes GROUP BY node_version, state ORDER BY node_version, state",
        "cluster_health",
    ),
    FixedStatement(
        "queries",
        "SELECT query_id, state, queued_time, analysis_time, planning_time, created, started, last_heartbeat, \"end\", error_type, error_code FROM system.runtime.queries WHERE (source IS NULL OR source <> 'dataobs-collector') AND created >= ? ORDER BY created, query_id LIMIT ?",
        "runtime_queries",
    ),
    FixedStatement(
        "tasks",
    ...
    FixedStatement(
        "materialized_views",
        "SELECT catalog_name, schema_name, name FROM system.metadata.materialized_views ORDER BY catalog_name, schema_name, name",
        "materialized_views",
    ),
)
