...
class CheckpointStore(Protocol):
    async def load(self, tenant_id: str, integration_id: str, capability: str) -> CollectionCheckpoint | None: ...
...
class InMemoryCheckpointStore:
    def __init__(self) -> None:
        self._items: dict[tuple[str, str, str], CollectionCheckpoint] = {}

    async def load(self, tenant_id: str, integration_id: str, capability: str) -> CollectionCheckpoint | None:
        return self._items.get((tenant_id, integration_id, capability))

    async def save(self, checkpoint: CollectionCheckpoint, *, expected_version: int | None) -> None:
        from .errors import CheckpointConflictError

        key = (checkpoint.tenant_id, checkpoint.integration_id, checkpoint.capability)
