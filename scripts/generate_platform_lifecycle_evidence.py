...
a = parser.parse_args()
if len(a.target_sha) != 40:
    raise SystemExit("target SHA must be exact")
...
for name in REPORTS:
    ...
    (a.output / f"{name}-report.json").write_text(
        json.dumps(
            {
                "schema_version": "1",
                "target_sha": a.target_sha,
                "status": status,
                "topology": "static_unit" if status == "pass" else "not_hosted",
                "generated_at": now,
            },
            indent=2,
        )
...
manifest = {
    "schema_version": "1",
    "target_sha": a.target_sha,
    "terminal_migration": "0027_platform_environment_tenant_multicluster_lifecycle",
    "workflow_run_id": os.getenv("GITHUB_RUN_ID", "local"),
    "workflow_attempt": os.getenv("GITHUB_RUN_ATTEMPT", "local"),
    "hosted_certification": "pending",
    "multi_cluster": "pending",
    "ha": "pending",
    "dr": "functional_simulation",
    "generated_at": now,
    "files": {},
}
for p in sorted(a.output.glob("*-report.json")):
