...
rows = []
for i, line in enumerate(SO_PATH.read_text().splitlines(), 1):
    if not line.strip():
        continue
    try:
        rows.append(json.loads(line))
    except Exception as e:
        raise SystemExit(f"Invalid NDJSON line {i}: {e}")
by_type = {}
for r in rows:
...
if miss:
    raise SystemExit(f"Missing data views: {sorted(miss)}")
...
for t in REQUIRED_DASHBOARDS:
    ...
    if not d:
        raise SystemExit(f"Missing dashboard: {t}")
    panels = json.loads(d["attributes"].get("panelsJSON", "[]"))
    if len(panels) < 5:
        raise SystemExit(f"Dashboard {t} has <5 panels")
    refs = {r["name"]: (r["type"], r["id"]) for r in d.get("references", [])}
    for p in panels:
        refn = p.get("panelRefName")
        if refn not in refs:
            raise SystemExit(f"Dashboard {t} panel missing reference: {refn}")
        if refs[refn] not in obj_by_key:
            raise SystemExit(f"Dashboard {t} reference object missing: {refs[refn]}")
