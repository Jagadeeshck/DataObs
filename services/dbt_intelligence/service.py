...
class DbtIntelligenceService:
        ...
        event_id = sha256(
            "\x1f".join(
                (
                    ...
                    project_id,
                    artifact_type,
                    artifact.envelope.artifact_schema_version,
                    artifact.envelope.invocation_id,
                    artifact.envelope.artifact_fingerprint,
                )
            ).encode()
        ).hexdigest()
        created = self.repository.append_artifact_event(
            event_id, {"event_id": event_id, "envelope": asdict(artifact.envelope)}
        )
        if created:
            self._project(artifact)
    ...
    def _project(self, artifact: NormalizedArtifact) -> None:
        ...
        existing = (
            getattr(self.repository, "get_project", lambda *_: None)(
                envelope.tenant_id, envelope.environment, envelope.project_id
            )
            or {}
        )
        for resource in artifact.resources:
            self.repository.upsert_resource_projection(
                envelope.tenant_id, envelope.environment, envelope.project_id, resource
            )
        counts = dict(existing.get("counts", {}))
        if artifact.resources:
            counts = dict(Counter(item["resource_type"] for item in artifact.resources))
        versions = dict(existing.get("artifact_schema_versions", {}))
        versions[envelope.artifact_type] = envelope.artifact_schema_version
        ...
        self.repository.upsert_project(
            DbtProject(
                ...
                envelope.dbt_version,
                versions,
                counts,
                envelope.generated_at if envelope.artifact_type == "manifest" else existing.get("latest_manifest_at"),
                envelope.generated_at if envelope.artifact_type == "run_results" else existing.get("latest_run_at"),
                health,
                repository=envelope.repository_ref,
            )
        )
