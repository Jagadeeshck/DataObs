...
STRONG_SIGNALS = frozenset(
    {"provider_throttling", "quota_breach", "capacity_rejection", "provisioned_capacity_exhausted"}
)
...
def evaluate_capacity(evidence: dict[str, Any], policy: CapacityPlanningPolicy | None = None) -> CapacityEvaluation:
    ...
    if utilisation is not None:
        state = (
            ...
            if utilisation >= policy.constrained_utilisation
            else CapacityState.WATCH if utilisation >= policy.watch_utilisation else CapacityState.HEALTHY
        )
    ...
    for raw in evidence.get("signals", ()):
        ...
        signals.append(
            SaturationSignal(
                signal_type,
                CapacityDimensionName(raw.get("dimension", CapacityDimensionName.BACKLOG.value)),
                raw.get("observed_value"),
                raw.get("threshold_or_limit"),
                raw.get("unit"),
                strength,
                raw.get("method", "provider_measured"),
                raw.get("first_observed_at", now),
                raw.get("last_observed_at", now),
    ...
    refs = tuple(
        dict.fromkeys(
            ([demand.evidence_ref] if demand else [])
            + ([limit.evidence_ref] if limit else [])
            + list(evidence.get("evidence_refs", ()))
        )
    )
    dimension = CapacityDimension(
        CapacityDimensionName.PROVIDER_INGRESS,
        demand.value if demand else None,
        limit.value if limit else None,
        utilisation,
        headroom.absolute,
        state,
        demand.method.value if demand else "unavailable",
        min(demand is not None and 1.0 or 0.0, limit.confidence if limit else 1.0),
        evidence.get("source_coverage", 0.0),
        tuple(code.removesuffix("_missing") for code in reasons),
        tuple(reasons),
        refs,
    )
    consumer = evaluate_consumer_capacity(
        evidence.get("arrival_rate"), evidence.get("processing_rate"), evidence.get("backlog")
    )
    ...
    if strong:
        candidates.append(
            CapacityBottleneck(
                "provider_ingress", max(item.confidence for item in strong), ("strong_saturation_evidence",), refs
            )
        )
    retention = evidence.get("retention_forecast")
    retention_plan = (
        RetentionCapacityPlan(
            retention, evidence.get("earliest_exhaustion_seconds"), retention.confidence, retention.data_coverage
        )
        if retention
        else None
    )
    missing = tuple(
        dict.fromkeys(
            reasons + (["checkpoints"] if system == "azure_event_hubs" and evidence.get("checkpoint_missing") else [])
        )
    )
    confidence = min(
        1.0,
        evidence.get("source_coverage", 0.0)
        * (0.7 if demand and demand.method.value == "provider_approximate" else 1.0),
    )
    return CapacityEvaluation(
        ...
        consumer,
        parallelism,
        evidence.get("partition_shard_pressure"),
        retention_plan,
        tuple(candidates or [CapacityBottleneck("unknown", confidence, ("no_bottleneck_evidence",), refs)]),
        confidence,
        missing,
        refs,
    )
...
def evaluation_document(
    evaluation: CapacityEvaluation, scope: dict[str, str], evaluation_id: str, fencing_token: int
) -> dict[str, Any]:
