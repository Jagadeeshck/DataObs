...
def calculate_asset_health(
    asset: Dict[str, Any], quality_runs: List[Dict[str, Any]], job_runs: List[Dict[str, Any]]
) -> str:
    ...
    for job in job_runs:
        if str(job.get("status", "")).lower() in {"delayed", "timeout"}:
            return "warning"
    if quality_runs and all(
        str(run.get("status", "")).lower() in {"pass", "passed", "success", "ok"} for run in quality_runs
    ):
        return "healthy"
    return "unknown"
...
class DataObservabilityService:
    ...
    def create_quality_check(self, check: Dict[str, Any]) -> Dict[str, Any]:
        ...
        check = {
            **check,
            "check_id": check.get("check_id") or _id("check"),
            "created_at": check.get("created_at", ts),
            "updated_at": ts,
        }
    ...
    def record_quality_run(self, run: Dict[str, Any]) -> Dict[str, Any]:
        run = {
            **run,
            "run_id": run.get("run_id") or _id("run"),
            "started_at": run.get("started_at", now_iso()),
            "finished_at": run.get("finished_at", now_iso()),
        }
    ...
    def ingest_lineage_edge(self, edge: Dict[str, Any]) -> Dict[str, Any]:
        edge_id = edge.get("edge_id") or stable_id(
            "lineage_edge",
            edge.get("source_asset_id"),
            edge.get("target_asset_id"),
            edge.get("job_id") or edge.get("job_name"),
        )
        ...
        document = {
            ...
            **edge,
            "edge_id": edge_id,
            "first_seen": (existing or {}).get("first_seen", edge.get("first_seen", observed_at)),
    ...
    def ingest_column_lineage_edge(self, edge: Dict[str, Any]) -> Dict[str, Any]:
        ...
        document = {
            **(existing or {}),
            **edge,
            "first_seen": (existing or {}).get("first_seen", edge.get("first_seen", now_iso())),
            "last_seen": edge.get("last_seen", now_iso()),
            "active": edge.get("active", True),
        }
    ...
    def update_asset_health(self, asset_id: Optional[str]) -> str:
        ...
        health = calculate_asset_health(
            asset, self.search_quality_runs(asset_id=asset_id), self.search_job_runs(asset_id=asset_id)
        )
    ...
    def ingest_openlineage_event(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        projected_quality_runs = [
            self.record_quality_run(run)
            for run in quality_assertion_runs(
                parsed["inputs"] + parsed["outputs"], run_id=parsed["run_id"], observed_at=parsed["event_time"]
            )
        ]
        return {
            "event_id": parsed["event_id"],
            ...
            "column_lineage_edges": projected_column_edges,
            "quality_runs": projected_quality_runs,
        }
