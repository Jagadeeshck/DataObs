...
Default telemetry path: **Elastic APM**.
  ...
  - Each stage (dataset_etl, ingest, spark_transform, lineage, observability)
    becomes a child span via the Elastic APM Python agent.
...
Stage added:
  dataset_etl  — downloads real public datasets (road-safety, air-quality,
                 local-authority) from data.gov.uk / ONS, auto-infers their
                 ES mapping, and bulk-indexes every row into:
                   dataobs-raw-road-safety
                   dataobs-raw-air-quality
                   dataobs-raw-local-authority
                 A summary document is written to dataobs-ingest-summary.
                 These indices are the primary data for Kibana dashboards.
"""
...
class DataObsPipelineRunner:
    ...
    def _run_stage(self, name: str, fn, attrs: Dict[str, Any] = {}):
        t0 = time.time()
        labels = {"stage": name, "tenant": self.tenant, "run_id": self.run_id, **attrs}
        try:
            ...
            logger.error("[Pipeline] Stage '%s' failed after %.2fs: %s", name, duration, exc)
            raise

    # ── Stages ─────────────────────────────────────────────────────────────

    def _stage_dataset_etl(self):
        """
        Download real public datasets from data.gov.uk / ONS, auto-infer
        their schema, and bulk-index every row into Elasticsearch.

        Indices produced (immediately visible in Kibana Discover):
          dataobs-raw-road-safety       — DfT road casualty statistics
          dataobs-raw-air-quality       — DEFRA AURN monitoring sites
          dataobs-raw-local-authority   — ONS LA district codes
          dataobs-ingest-summary        — per-run ingest summary
        """
    ...
    def _stage_spark_transform(self):
        try:
            ...
            spark = (
                ...
                .config("spark.jars.packages", "org.apache.spark:spark-sql_2.12:3.4.3")
                .getOrCreate()
            )

            with SparkApmInstrumentation(
                spark, apm=self._apm, job_name=f"dataobs-transform-{self.run_id}",
            ) as sapm:
                with sapm.stage("read_input", labels={"data.source": "inline"}):
                    data = [(i, float(i * 1.5), self.run_id) for i in range(100)]
                    df = spark.createDataFrame(data, ["record_id", "value", "run_id"])
                ...
                with sapm.stage("quality_check", labels={"quality.rule": "null_check"}):
                    ...
                    self._apm.label(quality_score=quality_score, null_count=null_count, row_count=row_count)
                    logger.info(
            ...
            logger.warning("[Pipeline] PySpark not available (%s) — running mock transform", exc)
    ...
    def _stage_mock_transform(self) -> int:
        ...
        mock_results = [
            ...
            for i in range(rows) if i * 1.5 > 10
        ]
    ...
    def _stage_lineage(self):
        from elasticsearch import Elasticsearch
        es = Elasticsearch(ES_HOST, basic_auth=(ES_USER, ES_PASS), verify_certs=False)
        es.index(index="dataobs-lineage", document={
            "@timestamp": datetime.utcnow().isoformat(),
            "run_id": self.run_id, "tenant": self.tenant,
            "source": "dataobs-test-data", "target": "dataobs-spark-results",
            "source_index": "dataobs-test-data", "sink_index": "dataobs-spark-results",
            "pipeline": self.SERVICE_NAME, "lineage_type": "transformation",
            "relation": "transformation", "row_count": 100,
        })
        for summary in self._etl_summaries:
            es.index(index="dataobs-lineage", document={
                ...
                "source_index": "external-url", "sink_index": summary["index"],
                "pipeline": self.SERVICE_NAME, "lineage_type": "ingest",
                "relation": "ingest", "row_count": summary["rows_ingested"],
                "dataset": summary["dataset"], "theme": summary.get("theme", "unknown"),
    ...
    def _stage_observability(self):
        ...
        ow.register_asset(
            asset_id="dataobs-test-data", name="dataobs-test-data",
            asset_type="elasticsearch_index", platform="elasticsearch",
            location=f"{ES_HOST}/dataobs-test-data",
            tags=["poc", "raw", "ingest"], row_count=50, column_count=5, reliability_score=98.0,
        )
        ow.register_asset(
            asset_id="dataobs-spark-results", name="dataobs-spark-results",
            asset_type="elasticsearch_index", platform="elasticsearch",
            location=f"{ES_HOST}/dataobs-spark-results",
            tags=["poc", "curated", "spark"], row_count=100, column_count=6, reliability_score=99.0,
        )
        for summary in self._etl_summaries:
            if summary["status"] == "ok":
                ow.register_asset(
                    asset_id=summary["index"], name=summary["index"],
                    asset_type="elasticsearch_index", platform="elasticsearch",
                    location=f"{ES_HOST}/{summary['index']}",
                    tags=["poc", "raw", "etl", summary.get("theme", "unknown")],
                    row_count=summary["rows_ingested"],
                    column_count=summary.get("columns", 0), reliability_score=99.0,
                )
                ow.emit_volume(
                    asset_id=summary["index"], asset_name=summary["index"],
                    row_count=summary["rows_ingested"], expected_min=1, expected_max=100_000,
                )
                ow.emit_freshness(
                    asset_id=summary["index"], asset_name=summary["index"],
                    last_seen=datetime.utcnow().isoformat(),
                    lag_seconds=int(summary["duration_seconds"]), sla_seconds=3600,
                )

        ow.emit_quality_check(
            run_id=self.run_id, asset_id="dataobs-spark-results",
            asset_name="dataobs-spark-results", check_name="row_count_min",
            check_type="volume", column=None, value=100.0, threshold=10.0,
            status="pass", expression="row_count >= 10", score=100.0,
            message="Row count above minimum",
        )
        ow.emit_quality_check(
            run_id=self.run_id, asset_id="dataobs-spark-results",
            asset_name="dataobs-spark-results", check_name="value_not_null",
            check_type="completeness", column="value", value=100.0, threshold=99.0,
            status="pass", expression="not_null(value) >= 99%", score=100.0,
            message="No null values detected",
        )
        ow.emit_quality_check(
            run_id=self.run_id, asset_id="dataobs-spark-results",
            asset_name="dataobs-spark-results", check_name="record_id_unique",
            check_type="uniqueness", column="record_id",
            value=2.0, threshold=0.0, status="warn", severity="warning",
            expression="duplicate_count(record_id) == 0", score=98.0,
            message="2 duplicate record_id values detected",
        )
        ow.emit_freshness(
            asset_id="dataobs-spark-results", asset_name="dataobs-spark-results",
            last_seen=datetime.utcnow().isoformat(), lag_seconds=30, sla_seconds=1800,
        )
        ow.emit_freshness(
            asset_id="dataobs-test-data", asset_name="dataobs-test-data",
            last_seen=datetime.utcnow().isoformat(), lag_seconds=7200, sla_seconds=3600,
        )
        ow.emit_volume(
            asset_id="dataobs-spark-results", asset_name="dataobs-spark-results",
        ...
        ow.emit_lineage(
            run_id=self.run_id, source="dataobs-test-data",
            target="dataobs-spark-results", relation="transformation",
            pipeline=self.SERVICE_NAME, row_count=100,
            fields=[
    ...
    def run(self):
        ...
        try:
            with self._apm.transaction(
                ...
                # Stage 0: download + ingest real datasets from data.gov.uk
                self._run_stage("dataset_etl", self._stage_dataset_etl)

                # Stage 1: synthetic test-data ingest (APM demo)
                self._run_stage("ingest", self._stage_ingest)

                # Stage 2: Spark transform on synthetic data
                self._run_stage("spark_transform", self._stage_spark_transform)

                # Stage 3: lineage (covers both real ETL + Spark paths)
                self._run_stage("lineage", self._stage_lineage)

                # Stage 4: observability metadata for all indices
