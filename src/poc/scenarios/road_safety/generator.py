...
def generate_scaled(
    seed_dir: Path, out_dir: Path, scale: str = "small", large_target_rows: int = 1_000_000
) -> Dict[str, Path]:
    ...
    cas_out = []
    for i in range(mul):
        a = dict(random.choice(accidents))
        a["accident_id"] = f"A{i+1}"
        if scale != "small":
            a["location_easting"] = str(int(a["location_easting"]) + random.randint(-3000, 3000))
        acc_out.append(a)
        v = dict(random.choice(vehicles))
        v["vehicle_id"] = f"V{i+1}"
        v["accident_id"] = a["accident_id"]
        veh_out.append(v)
        c = dict(random.choice(casualties))
        c["casualty_id"] = f"C{i+1}"
        c["vehicle_id"] = v["vehicle_id"]
        c["accident_id"] = a["accident_id"]
        cas_out.append(c)
    return {
