...
def run_road_safety_scenario(data_dir: Path, run_id: str, run_mode: str = "good") -> Dict[str, Any]:
    accidents=_read(data_dir/"accidents.csv"); vehicles=_read(data_dir/"vehicles.csv"); casualties=_read(data_dir/"casualties.csv")
    la={r["local_authority_code"]: r for r in _read(data_dir/"local_authorities.csv")}; w={r["weather_code"]:r for r in _read(data_dir/"weather_lookup.csv")}
    if run_mode=="bad":
        accidents=accidents[:max(1,int(len(accidents)*0.5))]
        if accidents: accidents.append(dict(accidents[0]))
        for a in accidents[:max(1,len(accidents)//2)]: a["local_authority_code"]=""
        if accidents: accidents[0]["severity_code"]="9"; accidents[0]["ingested_at"]=(datetime.now(timezone.utc)-timedelta(days=3)).isoformat()
        if casualties: casualties[0]["casualty_age"]="-5"; casualties[0]["accident_id"]="ORPHAN"
    a_map={a["accident_id"]:a for a in accidents}
    v_map={v["vehicle_id"]:v for v in vehicles}
    facts=[]
    for c in casualties:
        a=a_map.get(c["accident_id"]); v=v_map.get(c.get("vehicle_id",""))
    ...
    add("freshness_sla","fail" if any(datetime.fromisoformat(a["ingested_at"]).replace(tzinfo=timezone.utc) < datetime.now(timezone.utc)-timedelta(hours=24) for a in accidents if a.get("ingested_at")) else "pass",{})
