...
class OIDCValidator:
    ...
    def validate(self, token: str) -> dict[str, Any]:
        ...
        if not isinstance(claims.get(self.settings.subject_claim), str) or not claims[self.settings.subject_claim]:
            raise SecurityError("mandatory_claim_malformed", "Access token subject is malformed")
        if int(claims["exp"]) - int(claims["iat"]) > self.settings.maximum_token_lifetime_seconds:
            raise SecurityError("token_lifetime_exceeded", "Access token lifetime exceeds policy")
        ...
        if self.settings.allowed_authorised_parties and party not in self.settings.allowed_authorised_parties:
            raise SecurityError("authorised_party_mismatch", "Access token authorised party is invalid")
        token_type = claims.get("typ") or header.get("typ")
        if token_type and token_type not in self.settings.allowed_token_types:
