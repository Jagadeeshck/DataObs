...
def test_phase_adapter_rejects_unknown_phase():
    result = subprocess.run(["bash", "scripts/resilience/certification_phase.sh", "destroy-everything"],
                            text=True, capture_output=True)
...
def test_safety_guard_rejects_production_target():
    result = subprocess.run([
        sys.executable, "scripts/resilience/safety_guard.py",
        "--opt-in", "I_UNDERSTAND_DESTRUCTIVE_TEST_ONLY", "--environment", "test",
        "--synthetic-marker", "synthetic-a", "--sha", "a" * 40,
        "--elasticsearch-url", "https://prod.example.test:9200",
        "--allow-elasticsearch-host", "prod.example.test", "--kube-context", "kind-test",
        "--maximum-duration", "60", "--cleanup-plan", "delete-kind",
    ], text=True, capture_output=True)
