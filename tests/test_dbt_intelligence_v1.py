...
def manifest(**extra):
    value = {
        "metadata": {"dbt_schema_version": "https://schemas.getdbt.com/dbt/manifest/v12/manifest.json", "dbt_version": "1.10.0", "invocation_id": "inv-1"},
        "nodes": {
            "model.pkg.orders": {"resource_type": "model", "name": "orders", "package_name": "pkg", "database": "warehouse", "schema": "analytics", "config": {"materialized": "incremental", "contract": {"enforced": True}}, "columns": {"id": {"data_type": "integer"}}, "depends_on": {"nodes": ["source.pkg.raw.orders"]}},
            "test.pkg.not_null_orders_id": {"resource_type": "test", "name": "not_null_orders_id", "depends_on": {"nodes": ["model.pkg.orders"]}},
        },
        "sources": {"source.pkg.raw.orders": {"resource_type": "source", "name": "orders", "source_name": "raw", "columns": {"id": {"data_type": "integer"}}}},
        "semantic_models": {"semantic_model.pkg.orders": {"resource_type": "semantic_model", "name": "orders", "depends_on": {"nodes": ["model.pkg.orders"]}, "entities": [{"name": "order", "type": "primary"}]}},
        "metrics": {"metric.pkg.revenue": {"resource_type": "metric", "name": "revenue", "depends_on": {"nodes": ["semantic_model.pkg.orders"]}}},
        "saved_queries": {"saved_query.pkg.exec": {"resource_type": "saved_query", "name": "exec", "depends_on": {"nodes": ["metric.pkg.revenue"]}}},
        "exposures": {"exposure.pkg.dashboard": {"resource_type": "exposure", "name": "dashboard", "depends_on": {"nodes": ["model.pkg.orders"]}}},
        "groups": {"group.pkg.finance": {"resource_type": "group", "name": "finance"}},
        "unit_tests": {"unit_test.pkg.orders": {"resource_type": "unit_test", "name": "orders", "depends_on": {"nodes": ["model.pkg.orders"]}}},
    }
    value.update(extra)
    return value
...
def test_manifest_resource_families_and_canonical_identity():
    ...
    assert {item["resource_type"] for item in result.resources} == {"model", "source", "test", "semantic_model", "metric", "saved_query", "exposure", "group", "unit_test"}
...
@pytest.mark.parametrize("field", ["raw_sql", "compiled_sql", "raw_code", "compiled_code", "environment", "token", "private_key", "rows"])
def test_unsafe_artifact_fields_are_rejected(field):
    value = manifest(); value["nodes"]["model.pkg.orders"][field] = "SECRET sentinel"
    with pytest.raises(DbtArtifactError) as error: parse_artifact(value, envelope())
...
def test_unknown_and_malformed_schema_fail_before_partial_parsing():
    value = manifest(); value["metadata"]["dbt_schema_version"] = "https://schemas.getdbt.com/dbt/manifest/v99/manifest.json"
    with pytest.raises(DbtArtifactError) as error: parse_artifact(value, envelope())
...
def test_limits_are_enforced():
    with pytest.raises(DbtArtifactError) as error: parse_artifact(manifest(), envelope(), ArtifactLimits(max_resources=1))
    assert error.value.code == "resource_limit_exceeded"
    with pytest.raises(DbtArtifactError) as error: parse_artifact(manifest(), envelope(), ArtifactLimits(max_bytes=10))
    assert error.value.code == "artifact_too_large"


@pytest.mark.parametrize("kind,uri,key", [("run_results", "https://schemas.getdbt.com/dbt/run-results/v6/run-results.json", "executions"), ("catalog", "https://schemas.getdbt.com/dbt/catalog/v1/catalog.json", "catalog"), ("freshness", "https://schemas.getdbt.com/dbt/sources/v3/sources.json", "freshness")])
def test_other_artifact_families(kind, uri, key):
    document = {"metadata": {"dbt_schema_version": uri, "dbt_version": "1.10.0", "invocation_id": "inv"}, "results": [{"unique_id": "test.pkg.x", "status": "skipped", "message": "select secret from private"}]}
    if kind == "catalog": document = {"metadata": document["metadata"], "nodes": {"model.pkg.x": {"columns": {"id": {"index": 1, "type": "INT"}}, "stats": {"row_count": {"value": 0}, "unknown": {"value": "secret"}}}}}
...
def test_lineage_is_bounded_and_reconciled_without_deleting_provenance():
    ...
    pair = edges[0]
    reconciled = reconcile_lineage(edges, [{"source": pair["source"], "target": pair["target"]}, {"source": "x", "target": "y"}])
...
def test_idempotent_ingestion_and_tenant_isolation():
    repository = MemoryDbtIntelligenceRepository(); service = DbtIntelligenceService(repository)
