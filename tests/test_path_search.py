...
def edge(identifier: str, source: str, target: str, confidence: float = 1.0):
    return PathwayRouteEdge(id=identifier, source_node_id=source, destination_node_id=target, evidence=PathwayEvidence(evidence_type="trace_observed", confidence=confidence, observed_at=datetime.now(timezone.utc), coverage="complete"))
...
def test_search_is_bounded_cycle_safe_and_deterministic():
    ...
    edges = [edge("ab", "A", "B"), edge("bd", "B", "D"), edge("ac", "A", "C", .8), edge("cd", "C", "D"), edge("ba", "B", "A")]
...
def test_search_reports_partial_and_filters_low_confidence():
    ...
    routes, partial, excluded, _ = search_paths(nodes, [edge("ab", "A", "B", .3), edge("bc", "B", "C")], "A", "C", max_hops=2, max_paths=2, minimum_confidence=.5)
    assert not routes
    assert not partial
    assert excluded == 1
